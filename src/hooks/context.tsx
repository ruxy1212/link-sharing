...
export const MyAppProvider: React.FC<{ children: ReactNode }> = ({
  children,
}) => {
  const [uid, setUid] = useState<string>('') // the user's account id
  const [usersLinks, dispatch] = useReducer<
    React.Reducer<LinksState, LinksAction>
  >(linksReducer, []) // usersLinks contains ALL of the user's links
  ...
  const [openSaveChangesMessage, setOpenSaveChangesMessage] =
