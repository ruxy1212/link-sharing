...
const Input: FC<InputProps> = ({ type, label, name, placeholder, ...rest }) => {
  ...
  return (
    <fieldset className="w-full flex justify-between items-center flex-wrap">
      <label
        className="text-dl-dark-gray font-instrument text-xs md:text-base font-normal leading-[150%] w-full md:w-1/2 lg:w-5/12"
        htmlFor={label}
      >
        {label}
      </label>
      <div className="w-full md:w-1/2 lg:w-7/12 relative">
        ...
        />
        <div
          className="hidden absolute h-[15px] top-0 bottom-0 my-auto right-4 text-dl-red text-xs font-instrument font-normal leading-[150%]"
          ref={errorMessageRef}
        >
          Can&apos;t be empty
        </div>
