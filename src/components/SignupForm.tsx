...
export default function SignupForm() {
    ...
    const handleSubmit = async (e: FormEvent) => {
        ...
        if (passwordsDontMatch.current) passwordsDontMatch.current.style.display = 'none';
        if (notEightCharactersMessage.current) notEightCharactersMessage.current.style.color = '';

        if (userPassword !== userConfirmPassword) {
            if (passwordsDontMatch.current) passwordsDontMatch.current.style.display = 'block';
            setLoading(false);
            return;
        }

        if (userPassword.length < 8) {
            if (notEightCharactersMessage.current) notEightCharactersMessage.current.style.color = '#FF3939';
            setLoading(false);
            return;
        }
        ...
        try {
            ...
            console.error(error);
            if (emailAlreadyExists.current) emailAlreadyExists.current.style.display = 'block';
            setLoading(false);
        }
    };
    ...
    return (
        <form className="flex flex-col gap-6 mb-6" onSubmit={handleSubmit}>
            ...
            <Input 
                ...
                ref={confirmPassword}
            />
            <p className="text-dl-dark-gray text-sm font-sans font-normal leading-[150%]" ref={notEightCharactersMessage}>
                Password must contain at least 8 characters
            </p>
            <p className="hidden text-dl-red text-base font-sans font-normal leading-[150%]" ref={passwordsDontMatch}>
                Passwords do not match
            </p>
            <p className="hidden text-dl-red text-base font-sans font-normal leading-[150%]" ref={emailAlreadyExists}>
                Email already exists
            </p>
            <button disabled={loading} className="flex-shrink-0 rounded-lg h-12 bg-dl-purple text-dl-white text-base font-sans font-semibold leading-[150%] cursor-pointer hover:bg-dl-mid-purple hover:shadow-[0px_0px_32px_0px_rgba(99,60,255,0.25)] disabled:bg-dl-light-purple">
                {loading ? <CircularProgress className="text-dl-light-purple" color="secondary" /> : 'Create new account'}
            </button>
        </form>
    );
}
