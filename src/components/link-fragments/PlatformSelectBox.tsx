...
const PlaformSelectBox: FC<PlatformSelectBoxProps> = ({
  initialState,
  zIndex,
  linkId,
}) => {
  ...
  const initialPlatform = initialState
    ? initialState.toLowerCase().replace(' ', '').replace('.', '')
    : 'github'
  const { dispatch } = context
  const [platform, setPlatform] = useState<string>(initialState || 'Github')
  const [platformIcon, setPlatformIcon] = useState<string>(
    `/icons/select-icons/icon-${initialPlatform}.svg`
  )
      ...
      >
        ...
        {open ? (
          <Image
            ...
            height={20}
            width={20}
            className="w-4 object-contain absolute top-0 bottom-0 my-auto right-4"
          />
        ) : (
          <Image
            alt="up"
