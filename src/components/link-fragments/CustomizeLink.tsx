...
const CustomizeLink: React.FC<CustomizeLinkProps> = ({ link, index }) => {
  ...
  return (
    <fieldset
      className="p-5 flex flex-col gap-3 bg-dl-white-gray rounded-xl relative"
      key={link.id}
      name="linkContainer"
      id={link.id}
      ref={linkRef}
    >
      ...
      <button
        type="button"
        className="absolute top-5 right-[20px] bg-transparent text-dl-dark-gray font-instrument text-base font-normal leading-[150%] cursor-pointer hover:text-dl-red"
        onClick={removeLink}
        id={link.id}
      >
        Remove
